...
/* 
    ...
    string içersinden ayrýþtýrýlarak ortalmasýnýn alýnmasý.
    String split iþlemi.
 */
...
//istenilen sayýda string olarak aralarýna virgül konulmuþ int sayý üretir.
char* rasgete_str_sayi(int boyut) {

    int i=0, sayi;
    static char r_str_sayi[10*dizi_aleman_sayisi];
    char gecici[5]; //str sayilarý ve virgülü tutmak için.
    ...
    yeniSatir();
    //en sondaki virgülü kaldýrmak için
...
/*	FONK :int str_bol(char* str_ifade,char bol_kar, int bol_yer) 
    ...
    Açýklama    : Belli bir karakter ile ayrýlmýþ string ifadeden (sayýlarýn olduðu) istenilen sayma sýrasýndaki 
...
{ 
    ...
    printf("10 tane sayýyý aralarýna virgül koyarak giriniz : \n");

    //metod 1:
   /* char* rnd_str_sayilar = malloc(100 * sizeof(char));
    gets(rnd_str_sayilar);
    int* tam_sayilar = sayilar_dizi(rnd_str_sayilar);*/

    //metod 2:
    char giris_str_sayilar[100] = { 0 };
    gets(giris_str_sayilar);//
    int* tam_sayilar = sayilar_dizi(giris_str_sayilar);
